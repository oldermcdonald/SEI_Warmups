...
// add 'a' before elements starting with consonants
// add 'an' before elements starting with vowels

var vowelCheck = function(word) {
    for (i=0; i<word.length; i++) {
        var firsLet = word.charAt(0);
        if(firsLet == 'A' || firsLet == 'E'|| firsLet == 'I'|| firsLet == 'O'|| firsLet == 'U') {
            return(`an ${word}`);
        } else {
            return(`a ${word}`);
        }
...
var randomItem = function(arrayList) {
    var randomIndex = Math.floor(Math.random()*arrayList.length);
    return arrayList[randomIndex];
}

console.log('Generating new startup...')
console.log(`We're making ${vowelCheck(randomItem(thisArray))} for ${randomItem(thatArray)}`);
